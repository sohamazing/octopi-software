...
install_best_nvidia_driver() {
    ...
    if [[ "$current_driver_version" -eq 550 || "$current_driver_version" -eq 535 ]]; then
...
install_cuda_nvidia_linux() {
    if [[ "$(lsb_release -d)" =~ Ubuntu\ 22.04 ]]; then
        sudo apt-get update
        install_best_nvidia_driver
        install_cuda_toolkit
    else
        echo "CUDA and NVIDIA driver installation skipped. This script is designed for Ubuntu 22.04."
    fi
}
